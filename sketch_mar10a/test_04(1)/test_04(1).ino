...
const int servoPin4 = 12;  // 舵机信号引脚（固定，方便修改）
...
// 3. 初始化角度（上电复位姿态）
int baseAngle = 100;    // 底座正前
int bigArmAngle = 90;  // 大臂初始位
int smallArmAngle = 60; // 小臂联动位
int clawAngle = CLAW_OPEN; // 夹爪初始张开

void setArmPose(int base, int smallArm, int bigArm, int claw);
...
void setup() {
  ...
  setArmPose(baseAngle, smallArmAngle, bigArmAngle, clawAngle);
  //baseServo.write(baseAngle);
  //bigArmServo.write(bigArmAngle);
  //smallArmServo.write(smallArmAngle);
  ...
  Serial.println("指令说明：");
  Serial.println("w=大臂上升  s=大臂下降  a=底座左转  d=底座右转 i=小臂上升 k=小臂下降");
...
void setArmPose(int base, int smallArm, int bigArm, int claw) {
  // 安全限位（防止舵机堵转）
  smallArm = constrain(smallArm, BASE_MIN, BASE_MAX);
  ...
  if(baseAngle <= base)
  {
    for (pos = baseAngle; pos <= base; pos += 1) { 
    myservo.write(pos);  
  ...
  else
  {
   ...
   if(bigArmAngle <= bigArm)
  {
    for (pos = bigArmAngle; pos <= bigArm; pos += 1) { 
    myservo.write(pos);  
    delay(delayTime);     // 每步等20ms，让舵机完全到位
  }
  }
  ...
  if(smallArmAngle <= smallArm)
  {
    for (pos = smallArmAngle; pos <= smallArm; pos += 1) { 
    myservo.write(pos);  
    delay(delayTime);     // 每步等20ms，让舵机完全到位
  }
  }
  ...
  if(clawAngle <= claw)
  {
    for (pos = clawAngle; pos <= claw; pos += 1) { 
    myservo.write(pos);  
    delay(delayTime);     // 每步等20ms，让舵机完全到位
  }
  }
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    switch(cmd) {
      case 'w': // 大臂上升（夹爪水平抬高）
        setArmPose(baseAngle, smallArmAngle, bigArmAngle + step, clawAngle);
        break;
      case 's': // 大臂下降（夹爪水平降低）
        setArmPose(baseAngle, smallArmAngle, bigArmAngle - step, clawAngle);
        break;
      case 'i': // 小臂上升（夹爪水平抬高）
        setArmPose(baseAngle, smallArmAngle - step, bigArmAngle, clawAngle);
        break;
      case 'k': // 小臂下降（夹爪水平降低）
      setArmPose(baseAngle, smallArmAngle + step, bigArmAngle, clawAngle);
      break;
      case 'a': // 底座左转
        setArmPose(baseAngle - step, smallArmAngle, bigArmAngle, clawAngle);
        break;
      case 'd': // 底座右转
        setArmPose(baseAngle + step, smallArmAngle, bigArmAngle, clawAngle);
        break;
      case 'o': // 夹爪打开
        setArmPose(baseAngle, smallArmAngle, bigArmAngle, CLAW_OPEN);
        break;
      case 'c': // 夹爪闭合
        setArmPose(baseAngle, smallArmAngle, bigArmAngle, CLAW_CLOSE);
        break;
      case 'r': // 复位初始姿态
        setArmPose(100, 90, 60, CLAW_OPEN);
        break;
      default: // 未知指令提示
